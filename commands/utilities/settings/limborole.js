...
module.exports = {
    async execute(interaction) {
        ...
        guildsDB.get(q, [guild.id], async (err, row) => {
            ...
            try {
                if(row) {
                    guildsDB.run(`UPDATE Settings SET LimboRole = ? WHERE Guild = ?`, [role.id, guild.id]);
                } else {
            ...
            return interaction.reply(`The role ${role.name} has been set as the limbo role`, {ephemeral: true});
        })
