...
module.exports = {
    ...
    .addSubcommand(command =>
        command.setName('limborole')
        .setDescription('Set the role that is applied to a user when they\'re put in limbo')
    ...
    .addSubcommand(command => 
        command.setName('punishmentreason')
        .setDescription('The default reason given when none are provided')
        .addStringOption(option => 
            option.setName('reason')
            .setDescription('The default reason (EG: For being a muppet)')    
            .setRequired(true)
        )    
    ),
...
function GetLimboRoleId(guildId) {
    return new Promise((resolve, reject) => {
        ...
        guildsDB.get('SELECT * FROM Settings WHERE Guild = ?', [guildId], async (err, row) => {
            if(err) {   
                reject();
    ...
    punishmentsDB.all(q, [], async (err, rows) => {
        ...
        rows.forEach(row => {
            if((parseInt(row.WarningExpiry) - now) < 0) {
                punishmentsDB.run('DELETE FROM Warnings WHERE User = ? AND Guild = ?', [row.User, row.Guild])
            }
        });
    })
}