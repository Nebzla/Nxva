...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Take a user out of limbo, granting back any roles lost. Requires limbo role to be setup in settings')
        ...
        async execute(interaction, client) {
            ...
            let guildsDB = new sqlite3.Database('./Databases/guilds.db', sqlite3.OPEN_READONLY, (err) => {
                if(err) return console.error(err.message);
            });
            ...
            if(!roleId) return interaction.reply({content: 'You haven\'t setup a limbo role', ephemeral: true})
            

            const role = guild.roles.cache.get(roleId);
            if(!role) return interaction.reply({content: 'The role you previously used no longer exists, please reset it and try again.', ephemeral: true})





            q = 'SELECT * FROM Limbos WHERE Guild = ? AND User = ?'
            punishmentsDB.get(q, [guild.id, target.id], async (err, row) => {
                if(err) return console.error(err.message);

                if(!row) return interaction.reply({content: 'The person you are trying to free from limbo isn\'t in it!', ephemeral: true });
                else {
                   ...
                   const resp = await interaction.reply({
                    ...
                    try {
                        ...
                        if(c.customId === 'confirm') {
                            ...
                            punishmentsDB.run('DELETE FROM Limbos WHERE Guild = ? AND User = ?', [guild.id, target.id])
                            punishmentsDB.close();
