    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option => {
            return option.setName('duration')
                .setDescription('The duration to time the user out for (Max 28d)')
                .setRequired(true)
        })
        .addStringOption(option => {
            return option.setName('reason')
                .setDescription('The reason for timing them out, defaults to standard in settings')
        }),
        async execute(interaction, client) {
            ...
            if(!durationMs) return interaction.reply({content: 'Failed to parse timeout duration, make sure to follow the format', ephemeral: true})
            if(durationMs > ms('28d')) return interaction.reply({content: 'Sorry, you can\'t time someone out for more than 28 days!', ephemeral: true})
           ...
           `)
            ...
            try {
                ...
                if(c.customId === 'confirm') {
                    ...
                    guildTarget.timeout(durationMs)
                    .then(async () => {
