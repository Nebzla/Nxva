...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Puts a user in limbo, removing their roles until undone.')
        ...
        .addStringOption(option => {
            return option.setName('reason')
                .setDescription('The reason for putting them in limbo, defaults to standard in settings')
        }),
        async execute(interaction, client) {
            ...
            punishmentsDB.get(q, [guild.id, target.id], async (err, row) => {
                ...
                if(row) return interaction.reply({content: 'That user is already in limbo, did you mean to run /unlimbo?', ephemeral: true})
                else {
                    //Not In Limbo
                   ...
                   const resp = await interaction.reply({
                    ...
                    try {
                        ...
                        if(c.customId === 'confirm') {
                            ...
                            punishmentsDB.run('INSERT INTO Limbos(Guild,User,Reason,LostRoles) VALUES(?,?,?,?)',
