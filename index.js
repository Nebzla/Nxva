...
const dateString = `${timeInitialised.getDate()}/${timeInitialised.getMonth() + 1}/${timeInitialised.getFullYear()} ${timeInitialised.getHours()}:${timeInitialised.getMinutes()}:${timeInitialised.getSeconds()}`;
...
let guildsDB = new sqlite3.Database('./Databases/guilds.db', sqlite3.OPEN_CREATE | sqlite3.OPEN_READWRITE, (err) => {
    if(err) return console.error(err.message);
})

guildsDB.run(`CREATE TABLE IF NOT EXISTS Settings(Guild TEXT, WarningDuration TEXT, LimboRole TEXT, DefaultPunishReason TEXT)`)

punishmentsDB
//.run(`CREATE TABLE IF NOT EXISTS Mutes(Guild TEXT, User TEXT, IsPermanent INTEGER NOT NULL, MutedUntil TEXT, Reason TEXT)`)
.run(`CREATE TABLE IF NOT EXISTS Limbos(Guild TEXT, User TEXT, Reason TEXT, LostRoles TEXT)`)
.run(`CREATE TABLE IF NOT EXISTS Warnings(Guild TEXT, User TEXT, Warnings INTEGER, WarningExpiry TEXT, WarningReasons TEXT)`); // Will expire as defined by server owner from latest warning issued
punishmentsDB.close();
guildsDB.close();
