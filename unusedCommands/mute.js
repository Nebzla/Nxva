...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Mutes a user. Requires mute role to be setup in settings')
        ...
        .addStringOption(option => {
            return option.setName('reason')
                .setDescription('The reason for muting them, defaults to standard in settings')
        })
        .addStringOption(option => {
            return option.setName('duration')
                .setDescription('How long to mute the user for, will be permanent if no duration is specified')
        }),
        async execute(interaction, client) {

            let punishmentsDB = new sqlite3.Database('./Databases/punishments.db', sqlite3.OPEN_READWRITE, (err) => {
            ...
            if(durationStr !== 'Permanent') {
                ...
                return interaction.reply({content: 'There was an error parsing the duration (Example Formats: 60d, 5h, 20m, 1y)', ephemeral: true})
            ...
            punishmentsDB.get(q, [target.id, guild.id], async (err, row) => {
                ...
                if(row) {
                    punishmentsDB.close();
                    return interaction.reply({content: 'That person has already been muted, you can\'t mute them again!', ephemeral: true})
                } else {
...
**Expiration Date:** ${!isPermanent ? `${expiryDate.getDate()}/${expiryDate.getMonth() + 1}/${expiryDate.getFullYear()}` : 'Never'}
**Reason:** ${reason}
                    `)
                   ...
                   `)
                    ...
                    try {
                        ...
                        if(c.customId === 'confirm') {
                            ...
                            punishmentsDB.run(`INSERT INTO Mutes(Guild, User, IsPermanent, MutedUntil, Reason) VALUES(?,?,?,?,?)`, 
                            [guild.id, target.id, + isPermanent, !isPermanent ? expiryDate : null, reason]);
                            ...
                            await c.update({embeds: [buttons.embeds.cancelled], components: []});
                        }
