...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Unmutes a user. Requires mute role to be setup in settings')
        ...
        async execute(interaction) {
            ...
            punishmentsDB.get(q, [target.id, guild.id], async (err, row) => {
                ...
                if(!row) {
                    punishmentsDB.close();
                    return interaction.reply({content: 'That person hasn\'t been muted!', ephemeral: true});
                } else {
                    console.log(row);
...
**Expires On:** ${row.IsPermanent === 0 ? `${expiryDate.getDate()}/${expiryDate.getMonth() + 1}/${expiryDate.getFullYear()}` : 'Never'}
                    ...
                    .setDescription(`**User:** ${target.globalName}`)
                     ...
                     try {
                        ...
                        if(c.customId === 'confirm') {
                            ...
                            punishmentsDB.run(`DELETE FROM Mutes WHERE Guild = ? AND User = ?`, [guild.id, target.id]);
                            punishmentsDB.close();
